...
namespace Web.Areas.Employee.Controllers
{
    ...
    public class LeaveController : Controller
    {
        ...
        {
            var user = await _userManager.GetUserAsync(User);
            string apiUrl = $"https://localhost:7167/api/Leave?employeeId={user.Id}";
            ...
            if (ModelState.IsValid)
            {
                ...
                else
                {
                    ...
                    var response = await _httpClient.PostAsJsonAsync("https://localhost:7167/api/Leave", leaveViewModel);
        ...
        public async Task<IActionResult> DeleteLeave(int leaveId)
        {
            ...
            await _userManager.UpdateAsync(user);
            NameMethod(user);
            await _httpClient.DeleteAsync($"https://localhost:7167/api/Leave/{leaveId}");
