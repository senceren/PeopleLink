...
namespace Web.Areas.Employee.Controllers
{
    ...
    public class ExpenseController : Controller
    {
        ...
        public async Task<IActionResult> AllExpenses(string? status)
        {
            ...
            string apiUrl = $"https://peoplelinkapi.cerensenyazici.com/api/Expense?employeeId={employeeId}";
            string apiUrlWithStatus = $"https://peoplelinkapi.cerensenyazici.com/api/Expense?status={status}&employeeId={employeeId}";
            NameMethod(user);
        ...
        {
            var user = await _userManager.GetUserAsync(User);
            string apiUrl = $"https://peoplelinkapi.cerensenyazici.com/api/Expense?&employeeId={user.Id}";
            var expenses = await _httpClient.GetFromJsonAsync<List<GetExpenseViewModel>>(apiUrl);
            var expensesTypes = Enum.GetValues(typeof(ExpenseType)).Cast<ExpenseType>().ToList();
            var currencyTypes = Enum.GetValues(typeof(Currency)).Cast<Currency>().ToList();
            ViewBag.ExpenseTypes = expensesTypes;
            ...
            if (ModelState.IsValid)
            {
                ...
                await _httpClient.PostAsJsonAsync("https://peoplelinkapi.cerensenyazici.com/api/Expense/", expense);
        ...
        public async Task<IActionResult> DeleteExpense(int expenseId)
        {
            ...
            await _httpClient.DeleteAsync($"https://peoplelinkapi.cerensenyazici.com/api/Expense/{expenseId}");
